...
export async function createWish(formData: FormData) {
  ...
  const wish = await prisma.wish.create({ data: { userId: s.id, title, description, emoji, challenge } });
  await logAudit(prisma, { action: "WISH_CREATED", entityId: wish.id, entityType: "WISH", performedBy: s.id, affectedUserId: s.id, newValue: { title, challenge } });
...
export async function rejectWish(wishId: string, note: string) {
  ...
  const wish = await prisma.wish.findUnique({ where: { id: wishId } });
  if (!wish || wish.status !== "PENDING") throw new Error("Wish is not pending.");
  await prisma.wish.update({ where: { id: wishId }, data: { status: "REJECTED", decisionNote: note || null, decidedById: s.id, decidedAt: new Date() } });
  await logAudit(prisma, { action: "WISH_REJECTED", entityId: wishId, entityType: "WISH", performedBy: s.id, affectedUserId: wish.userId, newValue: { note } });
  await notify(prisma, { userId: wish.userId, type: "ANNOUNCEMENT", title: "Wish not approved", body: note ? `Reason: ${note}` : "Your wish was not approved this time.", link: "/wishing-tree" });
  revalidatePath("/wishing-tree");
}
...
export async function decideWishOutcome(wishId: string, grant: boolean, note: string) {
  ...
  await prisma.wish.update({ where: { id: wishId }, data: { status: grant ? "GRANTED" : "FAILED", decisionNote: note || null, decidedById: s.id, decidedAt: new Date() } });
  await logAudit(prisma, { action: grant ? "WISH_GRANTED" : "WISH_FAILED", entityId: wishId, entityType: "WISH", performedBy: s.id, affectedUserId: wish.userId, newValue: { note } });
  await notify(prisma, {
    userId: wish.userId, type: "ANNOUNCEMENT",
    title: grant ? "🎉 Wish GRANTED!" : "Challenge not passed",
    body: grant ? `Congratulations! Your wish "${wish.title}" has been granted. ${note ?? ""}`.trim() : (note ? `Reason: ${note}` : "The challenge wasn't quite met — try again!"),
    link: "/wishing-tree",
  });
  revalidatePath("/wishing-tree");
}
